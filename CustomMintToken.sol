...
contract CustomMintToken is MintableToken {
	string public constant name = "CustomMintToken";
...
/**
 * @title CustomMintTokenCrowdsale
 * @dev CustomMintTokenCrowdsale is a base contract for managing a token crowdsale.
 * Crowdsales have a start and end timestamps, where investors can make
 * token purchases and the crowdsale will assign them tokens based
 * on a token per ETH rate. Funds collected are forwarded to an owner
 * as they arrive.
 */
contract CustomMintTokenCrowdsale is Ownable {
  using SafeMath for uint256;
  ...
  // how many ETH cost 1000 CMT. rate = 1000 CMT/ETH. It's always an integer!
  //formula for rate: rate = 1000 * (CMT in USD) / (ETH in USD)
  uint256 public rate;
  ...
  function CustomMintTokenCrowdsale(uint256 _rate) public {
    require(_rate > 0);
	require (_rate < 1000);
  ...
  function createTokenContract() internal returns (CustomMintToken) {
    return new CustomMintToken();
  }
